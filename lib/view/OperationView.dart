            ...
            children: [
              Obx(() => Padding(
                ...
                child: ElevatedButton.icon(
                  icon: !oc.isLoading.value ?  Icon( Icons.send) :  CircularProgressIndicator(),
                  style: ElevatedButton.styleFrom(
                      backgroundColor:oc.color.value,
                      minimumSize : Size(160.0, 80.0),
                  ),
                  label: Text(!oc.isLoading.value ? "${oc.methodName}" : "Requesting..."),
                  onPressed: () async {
                    ...
                    if (res.code == 200) {
                      return await Get.dialog(SimpleDialog(
                          ...
                          children: [
                            ListTile(title: Text("1"),subtitle: Text("mess"),),
                          ],
                      ));
                    }else {
                      return await Get.dialog(AlertDialog(title: Text("error"),content: Text(res.message),));
                    }
